        ...
        <el-container class="center" style="border: #666666 solid 1px">
            <el-main>
                <el-form :model="loginForm" ref="user" label-width="100px" class="demo-ruleForm" id="loginDiv">
                    <el-form-item>
                        <el-header style="text-align: center;font-size: 25px;font-weight: bold;margin-left: -110px;margin-top: 30px">欢迎登录</el-header>
                    </el-form-item>
        ...
        login() {
            this.$http.post('/auth/login', this.loginForm).then(response => {
                if (response.data.code === 200) {
                    ...
                    this.$store.commit('setId', response.data.data.studentId);
